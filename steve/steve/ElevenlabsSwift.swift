//
//  ElevenlabsSwift.swift
//  steve
//
//  Modified by Ethan Sherbondy on 24/2/24.
//  because the original returned .mpg files which
//  did not play nicely with AVAudioPlayer by default
//
...
public class ElevenlabsSwift {
    ...
    public func textToSpeech(voice_id: String, text: String, model: String? = nil) async throws -> URL
    {
      ...
      // 2 == 75% latency optimization config
        let url = URL(string: "\(baseURL)/v1/text-to-speech/\(voice_id)?output_format=mp3_44100_32&optimize_streaming_latency=2")!
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue(elevenLabsAPI, forHTTPHeaderField: "xi-api-key")
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue("audio/mpeg3", forHTTPHeaderField: "Accept")
    ...
    private func saveDataToTempFile(data: Data) throws -> URL {
        ...
        // sherbondy: changed from .mpg output to .mp3 to be explicit > help
        // AVFoundation out with AVAudioPlayer format hinting
